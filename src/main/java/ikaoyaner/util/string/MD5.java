...
public class MD5 {
	...
	{
	  int[] bin = new int[(str.length()*chrsz>>5)+1];
	  int mask = (1 << chrsz) - 1;
	  for(int i = 0; i < str.length() * chrsz; i += chrsz){
		int charCodeAt = (int)str.charAt(i / chrsz);
	...
	public static int[] core_md5(int[] x, int len)
	{
	  ...
	  if(x.length < (((len + 64) >>> 9) << 4) + 14){
		  int[] newArr = new int[(((len + 64) >>> 9) << 4) + 14+1];
